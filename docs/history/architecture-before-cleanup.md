...
A React plant shop with a Node.js API and Cloud Firestore. The React storefront keeps
product CRUD, dynamic routing, Redux Toolkit state and EN/RU localization.

**Current layout:** `frontend/src` contains the Feature-Sliced Design tree; `backend/src`
contains the HTTP API, validation, authentication, and Firestore repositories. The old
`frontend/src/mocks` tree is test infrastructure only. Production requests go to Node.js.
...
- **RTK Query owns server state.** Anything that came from the Node.js API is cache, not state.
...
| 62  | Move the React app into `frontend/` and add a Node.js API in `backend/`                                | The browser no longer starts MSW. Vite proxies `/api` in development and Node.js serves the built frontend and API in production. Existing RTK Query endpoints keep their response shapes.                                                                                                                      |
| 63  | Firestore access uses a keyless Node.js API                                                            | The API uses Firestore REST with Firebase ID tokens for admin writes. Security Rules allow public catalog reads, restrict product writes to the verified admin email, and validate newsletter creates. The API also validates input.                                                     |
| 64  | Uploaded images use SHA-256 document IDs in Firestore                                                 | A hash alone cannot render an image. The database stores validated image bytes in a separate document, and the product stores a URL to the image endpoint. The 300 KB limit keeps the document below Firestore's size limit.                                                                                       |
| 65  | Google admin sign-in is manual and session memory only                                                | The user initiates a popup. The Node.js API checks the verified email, including `email_verified`, on every product write. A browser storage flag can no longer grant admin access.                                                                                                                              |
